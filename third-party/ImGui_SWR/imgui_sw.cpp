...
#pragma pack(pop)
  ...
  ImVec4 operator+(const ImVec4 &a, const ImVec4 &b) { return ImVec4{ a.x + b.x, a.y + b.y, a.z + b.z, a.w + b.w }; }
  ...
  Int as_int(float v) { return static_cast<Int>(std::floor(v * kFixedBias)); }
...
void paint_imgui(uint32_t *pixels, ImDrawData *drawData, const SwOptions &options)
{
  ...
  for (int i = 0; i < drawData->CmdListsCount; ++i) {
    paint_draw_list(target, drawData->CmdLists[i], options, &s_stats);
  }
}
